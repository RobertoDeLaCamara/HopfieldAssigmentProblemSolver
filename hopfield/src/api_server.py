...
def solve_assignment():
    ...
    try:
        ...
        if data is None:
            return (
                jsonify(
                    {"success": False, "error": "No JSON provided in request body"}
                ),
                400,
            )
        ...
        if "cost_matrix" not in data:
            return (
                jsonify({"success": False, "error": "Field 'cost_matrix' is required"}),
                400,
            )
...
def solve_batch():
    ...
    try:
        ...
        if not data or "problems" not in data:
            return (
                jsonify({"success": False, "error": "Field 'problems' is required"}),
                400,
            )
        ...
        if not isinstance(problems, list):
            return (
                jsonify({"success": False, "error": "Field 'problems' must be a list"}),
                400,
            )

        if len(problems) == 0:
            return (
                jsonify({"success": False, "error": "Problems list cannot be empty"}),
                400,
            )
        ...
        for problem in problems:
            ...
            try:
                ...
                results.append({"id": problem["id"], "success": True, "result": result})
            except Exception as e:
                results.append({"id": problem["id"], "success": False, "error": str(e)})
