...
# --- Tests for sort_missing_assignments (#10 fallback determinism) ---
# Generated by gemma4 via local Ollama delegation; verified passing.
from hopfield_solver import sort_missing_assignments


def test_sort_missing_empty_sets_n_3():
    assert sort_missing_assignments(set(), set(), 3) == [(0, 0), (1, 1), (2, 2)]


def test_sort_missing_fully_assigned():
    assert sort_missing_assignments({0, 1, 2}, {0, 1, 2}, 3) == []


def test_sort_missing_rows_02_cols_1_n_3():
    # missing_rows=[1], missing_cols=[0, 2] -> pair shorter
    assert sort_missing_assignments({0, 2}, {1}, 3) == [(1, 0)]


def test_sort_missing_rows_1_cols_02_n_3():
    # missing_rows=[0, 2], missing_cols=[1] -> pair shorter
    assert sort_missing_assignments({1}, {0, 2}, 3) == [(0, 1)]


def test_sort_missing_n_zero():
    assert sort_missing_assignments(set(), set(), 0) == []


def test_sort_missing_determinism():
    r1 = sort_missing_assignments({0, 2}, {1}, 3)
    r2 = sort_missing_assignments({0, 2}, {1}, 3)
    assert r1 == r2
